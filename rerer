...
1. Replace ServerUrl with ServerUrl from environment this process is being ran. Simply search and then replace. Here is list of Urls included in this base script:
											...
											2016 DEV
CAPMS Home                     https://capms-dev2.cms.gov
CMSNet Home                    https://cmsintranet.share-dev2.cms.gov
Share Home                     https://share-dev2.cms.gov
...
WHERE ServerUrl = 'https://share-dev2.cms.gov' and Date =
(
...
SELECT "Total Unique Users"
FROM UsageAndHealth_Statistics_Total_Unique_Users_Weekly with(nolock)
WHERE ServerUrl = 'https://cmsintranet.share-dev2.cms.gov' and Date =
(
	SELECT CONVERT(DATE,CURRENT_TIMESTAMP)
)
...
SELECT "Total Unique Users"
FROM UsageAndHealth_Statistics_Total_Unique_Users_Weekly with(nolock)
WHERE ServerUrl = 'https://capms-dev2.cms.gov' and Date =
(
	SELECT CONVERT(DATE,CURRENT_TIMESTAMP)
)
...
WHERE ServerUrl = 'https://share-dev2.cms.gov' and "Date Collected" =
(
	SELECT CONVERT(DATE,CURRENT_TIMESTAMP)
)
...
SELECT Day, "Day of Week", "Unique Users"
FROM UsageAndHealth_Statistics_Daily_Unique_Users with(nolock)
WHERE ServerUrl = 'https://cmsintranet.share-dev2.cms.gov' and "Date Collected" =
(
	SELECT CONVERT(DATE,CURRENT_TIMESTAMP)
)
...
SELECT Day, "Day of Week", "Unique Users"
FROM UsageAndHealth_Statistics_Daily_Unique_Users with(nolock)
WHERE ServerUrl = 'https://capms-dev2.cms.gov' and "Date Collected" =
(
	SELECT CONVERT(DATE,CURRENT_TIMESTAMP)
)
...
FROM
 ...
 SELECT TOP (5) [ID]
  ...
  WHERE WebapplicationUrl = 'https://share-dev2.cms.gov'
  ORDER BY ID desc
  )
...
FROM
 ...
 SELECT TOP (5) [ID]
      ...
      ,CONVERT(INT, [Users]) Users
  FROM [AST_Internal_UsageHealth].[dbo].[UsageAndHealth_Statistics_Top_Five_Site_URL]
  WHERE WebapplicationUrl = 'https://cmsintranet.share-dev2.cms.gov'
  ORDER BY ID desc
  )
...
FROM
 ...
 SELECT TOP (5) [ID]
      ...
      ,CONVERT(INT, [Users]) Users
  FROM [AST_Internal_UsageHealth].[dbo].[UsageAndHealth_Statistics_Top_Five_Site_URL]
  WHERE WebapplicationUrl = 'https://capms-dev2.cms.gov'
  ORDER BY ID desc
  )
